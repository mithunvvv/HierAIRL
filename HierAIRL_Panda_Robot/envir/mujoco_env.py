...
class MujocoEnv(object):
    ...
    def init(self, display=False):
        ...
        # gym.envs.registry.register("PandaPickAndPlace-v3")
        self.env = PandaPickAndPlaceEnv()
    ...
    def step(self, a):
        s, reward, terminate, info, is_success = self.env.step(a)
        if self.display:
            self.env.render()
    ...
    def state_action_size(self):
        if self.env is not None:
            s_dim = self.env.observation_space["observation"].shape[0]
