...
from sampler import Sampler, option_loop
...
def learn(config: Config, msg="default"):
    ...
    for i in range(n_epoch):
        ...
        if i % 1 == 0:
            ...
            il.policy.load_state_dict(il.policy.state_dict())
            il.policy.eval()
            traj_list = []
            for traj in range(50):
                traj_list.append(option_loop(env, il.policy, fixed=True))

            max_reward = float('-inf')
            for s_array, c_array, a_array, r_array in traj_list:
                avg_reward = r_array.sum().item() 
                print("reward after eval: ", r_array, avg_reward)
                max_reward = max(max_reward, avg_reward)

        print(f"{i}: max-reward={max_reward}")
        logger.log_train("max-reward", max_reward, i)
...
if __name__ == '__main__':
    ...
    wandb.init(project="Panda_Robot", sync_tensorboard=True)
