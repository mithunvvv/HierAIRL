...
class MHAOptionAIRL(torch.nn.Module):
    ...
    def step(self, sample_scar, demo_scar, n_step=10):
        ...
        for _ in range(n_step):
            ...
            for ind_p in inds.split(self.mini_bs):
                ...
                if len(ap_b.shape) != len(ae_b.shape):
                    breakpoint()
    ...
    def convert_demo(self, demo_sa):
        with torch.no_grad(): # important
            out_sample = []
            r_sum_avg = 0.
            for idx, (s_array, a_array) in enumerate(demo_sa):
                if s_array.shape[0] == 1:
                    continue 
                if s_array.shape[0] != a_array.shape[0]:
                    breakpoint()
